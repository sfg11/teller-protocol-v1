...
library LibDapps {
    ...
    // TODO: ?????????????????????????????????????????????????????
    /**
        @notice Grabs the curve lending pool address based on asset deposited
        @return ICurveLendingPool instance
     */
    function getCurveLendingPool() internal view returns (ICurveLendingPool) {
        return
            ICurveLendingPool(
                ICrvZap(0xA79828DF1850E8a3A3064576f380D90aECDD3359)
            );
    }
}
